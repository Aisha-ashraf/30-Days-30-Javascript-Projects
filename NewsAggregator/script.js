...
let isLoading = false;
let lasrarticleCount = 0;
function fetchNews(isSearching){
    if(isLoading) return;
    isLoading = true;
    let url;
    if(isSearching){
        const keyword = document.getElementById('.seachKeyword').value;
    ...
    fetch(url).then(response =>response.json()).then(data =>{
        ...
        if(articlesWithImage.length === 0 || articlesWithImage === lasrarticleCount){
            displayNOMoreNews();
            return;
        }
        lastarticleCount = articlesWithImage.length;
        articlesWithImage.foreach(article =>{
            const newsItem = `<div class = "newsItem" >
            <div class = 'newsImage>
